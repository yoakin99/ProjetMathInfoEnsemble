...
def screenshot():
    canvas.postscript(file="./image/canvas_output.ps", colormode='color')
    print("Canvas saved as 'canvas_output.ps'")
...
Screenshot_button = Button(root, text="Screenshot", width=20, command=screenshot, bg='#485e1a', font=buttonFont)
Screenshot_button.pack(side=TOP, padx=3, pady=5)

union = tk.BooleanVar(value=False)
intersection = tk.BooleanVar(value=False)
triangle1 = tk.BooleanVar(value=True)
triangle2 = tk.BooleanVar(value=True)
triangle3 = tk.BooleanVar(value=True)

checkbox_frame = tk.Frame(root)
checkbox_frame.pack(side=TOP, padx=5, pady=10)

checkboxUnion = tk.Checkbutton(
    checkbox_frame, 
    text="Union", 
    variable=union,
    command=lambda: faireUnion(triangle1.get(), triangle2.get() ,triangle3.get() ,union.get() , intersection.get()),
    bg='#cbf57d',
    font=buttonFont
)
checkboxUnion.pack(anchor=W)

checkboxIntersection = tk.Checkbutton(
    checkbox_frame, 
    text="Intersection", 
    variable=intersection,
    command=lambda: faireIntersection(triangle1.get(), triangle2.get() ,triangle3.get() ,intersection.get() , union.get()),
    bg='#cbf57d',
    font=buttonFont
)
checkboxIntersection.pack(anchor=W)

checkboxTriangle1 = tk.Checkbutton(
    checkbox_frame, 
    text="Triangle 1", 
    variable=triangle1,
    command=lambda: faireUnion(triangle1.get(), triangle2.get() ,triangle3.get() ,union.get() , intersection.get()),
...
checkboxTriangle1.pack(anchor=W)

checkboxTriangle2 = tk.Checkbutton(
    checkbox_frame, 
    text="Triangle 2", 
    variable=triangle2,
    command=lambda: faireUnion(triangle1.get(), triangle2.get() ,triangle3.get() ,union.get() , intersection.get()),
...
checkboxTriangle2.pack(anchor=W)

checkboxTriangle3 = tk.Checkbutton(
    checkbox_frame, 
    text="Triangle 3", 
    variable=triangle3,
    command=lambda: faireUnion(triangle1.get(), triangle2.get() ,triangle3.get() ,union.get() , intersection.get()),
...
checkboxTriangle3.pack(anchor=W)

def afficherTriangles(etat1, etat2, etat3):
    if etat1 and len(liste_ensemble) > 0:
        afficher_ens(liste_ensemble[0], liste_couleur_tkinter[randint(0,5)])
    if etat2 and len(liste_ensemble) > 1:
        afficher_ens(liste_ensemble[1], liste_couleur_tkinter[randint(0,5)])
    if etat3 and len(liste_ensemble) > 2:
        afficher_ens(liste_ensemble[2], liste_couleur_tkinter[randint(0,5)])


def faireUnion(etat1, etat2, etat3, etat4, etat5) :
    affichage()
    if (etat4 == True) :
        intersection.set(False)
        listeUnion = []
        if etat1 and len(liste_ensemble) > 0:
            listeUnion.append(liste_ensemble[0])
        if etat2 and len(liste_ensemble) > 1:
            listeUnion.append(liste_ensemble[1])
        if etat3 and len(liste_ensemble) > 2:
            listeUnion.append(liste_ensemble[2])
            
        while (1<len(listeUnion)) :
            ens1 = listeUnion.pop()
            ens2 = listeUnion.pop()
            ensInt = ens1.union(ens2)
            listeUnion.append(ensInt)
        if (len(listeUnion) == 1) :
            afficher_ens(listeUnion[0],liste_couleur_tkinter[randint(0,5)])

    elif etat5 == True:
        faireIntersection(etat1, etat2, etat3, etat5, etat4)
    
    else :
        afficherTriangles(etat1, etat2, etat3)
    



def faireIntersection(etat1, etat2, etat3, etat4, etat5) :
    affichage()
    if (etat4 == True) :
        union.set(False)
        listeIntersection = []
        if etat1 and len(liste_ensemble) > 0:
            listeIntersection.append(liste_ensemble[0])
        if etat2 and len(liste_ensemble) > 1:
            listeIntersection.append(liste_ensemble[1])
        if etat3 and len(liste_ensemble) > 2:
            listeIntersection.append(liste_ensemble[2])
            
        while (1<len(listeIntersection)) :
            ens1 = listeIntersection.pop()
            ens2 = listeIntersection.pop()
            ensInt = ens1.intersection(ens2)
            listeIntersection.append(ensInt)
        if (len(listeIntersection) == 1) :
            afficher_ens(listeIntersection[0],liste_couleur_tkinter[randint(0,5)])
